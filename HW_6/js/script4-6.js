// Массив watchList содержит объекты с информацией о нескольких фильмах. Используйте map, чтобы извлечь заголовок и рейтинг из списка наблюдения и сохранить новый массив в переменной рейтинга.
...
var filteredfilms = watchList.filter(item => item["Director"] == "Christopher Nolan").map((item) =>({
    Title: item["Title"],
...
var ratingList = filteredfilms.map(x =>  x.imdbRating);

var numberedList = ratingList.map(Number);
console.log(numberedList);

var AverageRating = numberedList.reduce((total, rating) => (total + rating), 0) / numberedList.length;

console.log("Average IMDB Rating: " + AverageRating); 